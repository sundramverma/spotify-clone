...
const DisplayAlbum = ({ album }) => {
    ...
    return albumData ? (
        <>
            ...
            <hr />
            {songsData
                .filter((item) => item.album === album?.name) // ✅ Fixed: Optional chaining
                .map((item, index) => (
                    <div
                        key={item._id || index} // ✅ Better key
                    ...
                    >
                        <div className='flex items-center w-10'>
                            {track?._id === item._id && playStatus ? (
                                <img 
                                    className={hoveredSongId === item._id ? "m-auto w-5 mr-4" : 'object-contain m-auto mr-4 w-5 h-5'} 
                        ...
                        <p className={`text-[15px] font-bold ${track?._id === item._id && playStatus ? 'text-green-500' : ''}`}>
                            {item.name}
                        </p>
